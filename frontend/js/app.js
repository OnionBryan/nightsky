...
class App {
    ...
    async init() {
        ...
        try {
            ...
            // Initialize 3D globe (lazy — actual init deferred until first tab click)
            ...
            await this.fetchPolarCrossings();

            // Draw equator crossing markers
            this.orbitRenderer.drawEquatorCrossings(this.polarCrossings);
    ...
    async fetchPolarCrossings() {
        try {
            const response = await fetch(`${API_BASE}/polar-crossings?satellite=${this.currentSatellite}&hours=24`);
            if (!response.ok) {
                this.polarCrossings = null;
                return;
            }
            this.polarCrossings = await response.json();
        } catch (error) {
            console.error('Failed to fetch polar crossings:', error);
            this.polarCrossings = null;
        }
    }
    ...
    async update() {
        // Skip updates during active playback (playback drives its own ticks)
        if (this.playback && this.playback.active) return;

        // Update day/night terminator (2D modes only)
        if (this.terminator && this.currentMode !== 'globe') {
            this.terminator.update();
        }

        if (this.currentMode === 'constellation') {
            await this.updateConstellation();
        } else if (this.currentMode === 'globe') {
            await this.update3DGlobe();
        } else if (this.currentMode !== 'coverage') {
    ...
    async update3DGlobe() {
        ...
        const color = this.getSatelliteColor(this.currentSatellite);
        const name = this.getSatelliteName(this.currentSatellite);
        this.globe3d.updateSatellite(pos, color, name);

        // Past trail from history buffer
        const past = this.orbitRenderer.getHistory().map(h => ({
            lat: h.lat, lon: h.lon, alt: pos.altitude_km
        }));

        // Future prediction from pre-fetched track
        const now = new Date();
        const future = (this.trackData || [])
            .filter(p => new Date(p.time) > now)
            .map(p => ({ lat: p.lat, lon: p.lon, alt: p.alt || pos.altitude_km }));

        this.globe3d.updateTrack(past, future, color);
        this.globe3d.updateSwath(pos.latitude, pos.longitude);
        this.updatePositionDisplay(pos);
    }
    ...
    setupEventListeners() {
        ...
        // Projection toggle (Polar <-> Equirectangular)
        const toggleProjectionBtn = document.getElementById('btn-toggle-projection');
        if (toggleProjectionBtn) {
            toggleProjectionBtn.addEventListener('click', () => this.toggleProjection());
        }
        ...
        if (playbackToggle && playbackPanel) {
            playbackToggle.addEventListener('click', () => {
                const open = !playbackPanel.classList.contains('hidden');
                playbackPanel.classList.toggle('hidden', open);
                playbackToggle.classList.toggle('panel-open', !open);
                if (!open) {
                    // Pre-fill start time to now minus 3 hours
                    const pbStart = document.getElementById('pb-start');
                    if (pbStart && !pbStart.value) {
                        const d = new Date(Date.now() - 3 * 3600000);
                        pbStart.value = d.toISOString().slice(0, 16);
                    }
                }
            });
        }
        ...
        if (pbPlayBtn) {
            pbPlayBtn.addEventListener('click', () => {
                if (this.playback.active && this.playback.isPlaying) {
                    this.playback.pause();
                    pbPlayBtn.textContent = '▶ Play';
                } else if (this.playback.active && !this.playback.isPlaying) {
        ...
        if (pbStopBtn) {
            pbStopBtn.addEventListener('click', () => {
                this.playback.stop();
                this._pbTrack = null;
                if (pbPlayBtn) pbPlayBtn.textContent = '▶ Play';
                const pbSimtime = document.getElementById('pb-simtime');
                if (pbSimtime) pbSimtime.textContent = '--';
                const pbScrub = document.getElementById('pb-scrub');
                if (pbScrub) pbScrub.value = 0;
            });
        }
        ...
        if (pbScrub) {
            pbScrub.addEventListener('input', () => {
                if (!this.playback.active || !this.playback.windowStart || !this.playback.windowEnd) return;
                const t = this.playback.windowStart.getTime() +
                    (pbScrub.value / 1000) * (this.playback.windowEnd - this.playback.windowStart);
                this.playback.scrubTo(new Date(t));
            });
        }
        ...
        if (pbSpeed) {
            pbSpeed.addEventListener('change', () => {
                this.playback.setSpeed(Number(pbSpeed.value));
            });
        }
    ...
    switchMode(mode) {
        ...
        // Toggle 2D map vs 3D globe containers
        const mapContainer = document.getElementById('map-container');
        const globeContainer = document.getElementById('globe-container');
        if (mapContainer) mapContainer.style.display = mode === 'globe' ? 'none' : '';
        if (globeContainer) globeContainer.style.display = mode === 'globe' ? 'flex' : 'none';

        if (mode !== 'globe') {
            // Clear previous 2D visuals
        ...
        if (mode === 'live') {
            ...
            this.orbitRenderer.clearHistory();
            this.updateLegend();
            // Init on first use (container must be visible before globe.gl creates its canvas)
            setTimeout(() => {
                this.globe3d.init();
                this.globe3d.resize();
            }, 50);
    ...
    async toggleProjection() {
        const newType = this.projection.projectionType === 'polar'
            ? 'equirectangular'
            : 'polar';

        const btn = document.getElementById('btn-toggle-projection');
        if (btn) {
            btn.classList.toggle('active', newType === 'equirectangular');
            btn.textContent = newType === 'equirectangular' ? 'E' : 'P';
            btn.title = newType === 'equirectangular'
                ? 'Switch to Polar projection (M)'
                : 'Switch to Flat Map projection (M)';
        }

        // Switch the D3 projection — this redraws base + land
        await this.projection.setProjectionType(newType);

        // Downstream renderers hold stale references to path — refresh them
        this.orbitRenderer.path = this.projection.getPath();
        this.spotlight.path = this.projection.getPath();
        this.terminator.path = this.projection.getPath();

        // Re-render equator crossings
        this.orbitRenderer.drawEquatorCrossings(this.polarCrossings);
    ...
    async _startPlayback() {
        ...
        const startStr = pbStart ? pbStart.value : null;
        if (!startStr) {
            this.showError('Please select a start time for playback.');
            return;
        }

        const startDate = new Date(startStr);
        if (isNaN(startDate)) {
            this.showError('Invalid start time.');
            return;
        }

        const durationMin = pbDuration ? parseInt(pbDuration.value) : 360;
        const speed = pbSpeed ? parseInt(pbSpeed.value) : 60;

        this.showLoading(true, 'Fetching historical track...');
        try {
            const endDate = new Date(startDate.getTime() + durationMin * 60000);
            const resp = await fetch(
                `${API_BASE}/track?satellite=${this.currentSatellite}` +
                `&start=${startDate.toISOString()}&end=${endDate.toISOString()}&step=30`
            );
            const data = await resp.json();
            this._pbTrack = data.positions || [];
            this.showLoading(false);
        } catch (e) {
            this.showLoading(false);
            this.showError('Failed to fetch historical track data.');
            return;
        }

        if (!this._pbTrack.length) {
            this.showError('No track data returned for this time range.');
            return;
        }

        this.playback.start(startDate, durationMin, speed);
    }

    _onPlaybackTick(simTime) {
        if (!this._pbTrack || !this._pbTrack.length) return;

        // Find the track point closest to simTime
        let closest = this._pbTrack[0];
        let minDiff = Math.abs(new Date(closest.time) - simTime);
        for (const p of this._pbTrack) {
            const diff = Math.abs(new Date(p.time) - simTime);
        ...
        const pos = {
            ...
            altitude_km: closest.alt || 824,
            velocity_km_s: 7.6
        };

        const color = this.getSatelliteColor(this.currentSatellite);
        const idx = this._pbTrack.indexOf(closest);
        const past = this._pbTrack.slice(Math.max(0, idx - 60), idx + 1);
        const future = this._pbTrack.slice(idx + 1, Math.min(this._pbTrack.length, idx + 60));

        if (this.currentMode === 'globe') {
            this.globe3d.updateSatellite(pos, color, this.getSatelliteName(this.currentSatellite));
            this.globe3d.updateTrack(
                past.map(p => ({ lat: p.lat, lon: p.lon, alt: p.alt || 824 })),
                future.map(p => ({ lat: p.lat, lon: p.lon, alt: p.alt || 824 })),
                color
            );
            this.globe3d.updateSwath(pos.latitude, pos.longitude);
        } else if (this.currentMode !== 'coverage') {
            this.orbitRenderer.clearTrails();
            const history = past.map(p => ({ lat: p.lat, lon: p.lon }));
            this.orbitRenderer.drawFadingTrail(history, color);
            if (future.length > 1) this.orbitRenderer.drawPrediction(future, color);
        ...
        // Update scrubber + sim time display
        const progress = this.playback.getProgress();
        const pbScrub = document.getElementById('pb-scrub');
        if (pbScrub) pbScrub.value = Math.round(progress * 1000);

        const pbSimtime = document.getElementById('pb-simtime');
        if (pbSimtime) {
            pbSimtime.textContent = simTime.toISOString().replace('T', ' ').slice(0, 19) + ' UTC';
        }

        const timeEl = document.getElementById('time-display');
        if (timeEl) {
            timeEl.textContent = simTime.toISOString().replace('T', ' ').slice(0, 19) + ' UTC (SIM)';
        }
    }
