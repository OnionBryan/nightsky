...
async function reverseGeocode(lat, lon) {
    try {
        const resp = await fetch(
            ...
            { headers: { 'User-Agent': 'NightSkyPlanner/1.0' } }
        );
        if (resp.ok) {
            const data = await resp.json();
            const addr = data.address || {};
            PlannerState.location.name = addr.city || addr.town || addr.village ||
                addr.county || data.display_name || `${lat.toFixed(2)}°, ${lon.toFixed(2)}°`;
        } else {
...
function updateTonightList() {
    ...
    Elements.tonightList.innerHTML = topObjects.map(obj => `
        <div class="object-card" data-type="${obj.type}" data-id="${obj.id}" data-catalog="${obj.catalog === 'M' ? 'messier' : 'caldwell'}">
            <div class="object-header">
                <span class="object-name">${obj.id} - ${obj.name || ''}</span>
    ...
    document.querySelectorAll('.object-card').forEach(card => {
        card.addEventListener('click', () => {
            ...
            const catalog = card.dataset.catalog || (card.querySelector('.object-name').textContent.startsWith('M') ? 'messier' : 'caldwell');
...
function showSearchSuggestions(suggestions) {
    ...
    dropdown.innerHTML = suggestions.map(obj => `
        <div class="suggestion" data-id="${obj.id}">
            <span class="suggestion-id">${obj.id}</span>
...
function findObjectInCatalogs(query) {
    ...
    // Check Messier — JSON uses "M1" string format
    ...
    // Check Caldwell — JSON uses "C1" string format
...
function renderCatalogList(catalogName, listElement, progressElement) {
    ...
    // Update progress
    if (progressElement) {
        const percent = catalog.length > 0 ? (observed.size / catalog.length * 100).toFixed(0) : 0;
