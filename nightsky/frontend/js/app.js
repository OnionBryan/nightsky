...
window.NightSky = (() => {
    ...
    return {
        ...
        features: {
            ...
            initAll() { Object.values(registry).forEach(f => f.init && f.init()); },
            notifyLocationChange() {
                Object.values(registry).forEach(f => f.onLocationChange && f.onLocationChange(state.latitude, state.longitude));
            },
            notifyTimeChange() {
...
function setupEventListeners() {
    ...
    document.getElementById('projection-select').addEventListener('change', () => {
        const proj = document.getElementById('projection-select').value;
        const isFreeLook = (proj === 'gnomic');
        // Gnomic projection is full-sky drag — ground/cardinals don't apply
        if (isFreeLook) {
            document.getElementById('opt-ground').checked = false;
            document.getElementById('opt-cardinals').checked = false;
        }
        reinitPlanetarium();
        updateInfoPanel();
    });
...
function adjustTime(minutes) {
    ...
    // Update display immediately
    updateTimeDisplay();
    window.NightSky.features.notifyTimeChange();
}
...
function updatePlanetariumLocation() {
    reinitPlanetarium();
    updateInfoPanel();
    window.NightSky.features.notifyLocationChange();
}
...
function setupTelescopeControls() {
    ...
    // Telescope button in header — opens Aladin at current sky center
    const telescopeBtn = document.getElementById('telescope-btn');
    if (telescopeBtn) {
        telescopeBtn.addEventListener('click', () => {
            // Get the center of the current VirtualSky view
            const p = state.planetarium;
            if (p) {
                // VirtualSky az/alt center → approximate RA/Dec
                const ra = p.ra_off || 180;
                const dec = p.dc_off || 0;
                openTelescopeView(ra, dec);
            } else {
                // Fallback: open at Orion Nebula
                openTelescopeView(83.82, -5.39);
            }
        });
    }
}
...
function setupRedlightToggle() {
    const btn = document.getElementById('redlight-toggle');
    if (!btn) return;

    // Restore from localStorage
    if (localStorage.getItem('nightsky-redlight') === 'true') {
        document.body.classList.add('redlight');
    }

    btn.addEventListener('click', () => {
        document.body.classList.toggle('redlight');
        const isRed = document.body.classList.contains('redlight');
        localStorage.setItem('nightsky-redlight', isRed);
    });
}
...
S(document).ready(function() {
    ...
    window.NightSky.features.initAll();
